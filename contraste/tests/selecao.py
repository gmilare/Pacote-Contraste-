...
for i in range(0,mapbiomas.shape[0]):
  for j in range(0,mapbiomas.shape[1]):
    if mapbiomas[i][j] == 24:
      ...
      fraizred[i][j] = red[i][j]
      fraiznir[i][j] = nir[i][j]
      fraizswir[i][j] = swir[i][j]
...
#Comparando os contrastes por classe -- as demais classes mantidas sem contraste
